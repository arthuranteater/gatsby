...
`allMarkdownRemark(sort: { fields: frontmatter___date, order: DESC })`. _Note: There are 3 underscores between `frontmatter` and `date`._ Save and restart the development server and the sort order should be fixed.
